...
class HardeningDocumentationBoundaryTest {
  ...
  fun `installed plugin owns mechanics and consumer docs own local evidence`() {
    ...
    assertTrue(
      hardening.contains("Consumer notes and `config/pitest/README.md` should") &&
          ...
          hardening.contains("nothing checks that copy") &&
          hardening.replace(Regex("\\s+"), " ").contains("Nothing checks the copy") &&
      ...
      "the agent block must be an unchecked copy that keeps local notes outside it",
    )
    assertFalse(hardening.contains("agentsTemplateInSync"), "the sync gate is gone")
    assertFalse(hardening.contains("hardeningAgentTemplateDiff"), "the diff task is gone")
    assertFalse(hardening.contains("hardening-template sha256:"), "no digest marker remains")
  ...
  fun `template upgrade prose routes to installed project-qualified task authority`() {
    ...
    assertTrue(
      compactReadme.contains("Treat the installed plugin as the task authority") &&
          compactReadme.contains("./gradlew :module:hardeningHelp") &&
          compactReadme.contains("re-copy the block printed by its project-qualified `hardeningAgentTemplate`") &&
          compactReadme.contains("Nothing gates `check` on that copy") &&
    ...
    assertFalse(readme.contains("agentsTemplateInSync"))
    assertFalse(readme.contains("hardeningAgentTemplateDiff"))
