...
internal data class PrunePreviewState(
  ...
  init {
    ...
    require((qualifies && matchingObservations >= 1) || (!qualifies && matchingObservations == 0)) {
      "qualifying prune-preview state must have observations; ineligible state must have none"
    }
  ...
  fun render(): String = buildString {
    ...
    appendLine("# qualifies $qualifies")
    candidates.forEach(::appendLine)
  }

  companion object {
    const val FORMAT_HEADER = "# prune preview format 1"
    private val SHA256 = Regex("[0-9a-f]{64}")

    fun parse(text: String): PrunePreviewState {
      ...
      require(lines[0] == FORMAT_HEADER) { "unsupported prune-preview format" }
      ...
      val qualifiesText = field(5, "# qualifies ")
      require(qualifiesText == "true" || qualifiesText == "false") {
        "prune-preview qualifies must be true or false"
      }
