...
`AGENTS.md` carries a copy of the short agent-instructions block below plus
repository-specific facts; nothing checks that copy.
...
digest, and the plugin binary actually loaded by every consumer *(casebook: the reviewed
repository that ran different bytes)*. An explicit local run is valid; it needs neither
a scheduled workflow nor an arbitrary soak window. A receipt
must bind the complete expected roster, refuse a stale shared plugin binary, and say
which recorded checkouts it revalidated *(casebook: the static local coordinate that
changed during certification)*. Keep evidence outside the tree it certifies
...
section *(casebook: the certification that the release path could not see)*.
...
an OS lock and writes a `.running` sentinel before PIT *(casebook: the clean command that
erased certification)*. The prior TSV remains available as
...
- **Tune the per-test timeout to the suite's real runtimes** — PIT's default
  ...
  suspecting the code *(casebook: timeout budgets sized to the tests, not the
  default)*.
...
certification populations share one licensed tool identity *(casebook: the fresh flag
that changed the population)*. Completed evidence binds
...
a mutant is still timed out, newly killed, newly surviving, or absent.
Before adding, removing, or relabelling an accepted-baseline or timeout-audit row, run
`pitest<Suite> -PnoMutationHistory`. Explicit `-PstrictTimeoutAudit` disables
...
3. Review its retained/added counts and the complete diff, then triage the additions.
   ...
   that every preserved row was generated by that toolchain *(casebook: the toolchain
   sidecar that appeared to timestamp every row)*. The safe-superset
...
lets two siblings of one key carry two different family labels *(casebook: the union
write that deduped siblings)*. All baseline
...
`# refactor` tombstone inside the accepted record *(casebook: the adoption that proved
the JAR but not its source)*.
...
- **Run-to-run drift is announced automatically.** The verify stashes each
  ...
  unaudited-row warning, while the drift output retains the changed coordinate,
  multiplicity, and every line-full candidate for review. The line is diagnostic
  metadata, not an authorization boundary: ordinary formatting, inserted methods,
  and line reflow must not warn, fail, or require a record edit. The two
...
- **Prefer removing the cause**: a fake collaborator that turns a would-be
  ...
  machine-readable comment category, optional `# line` diagnostic metadata,
  ...
  - `cause:liveness` is the only admissible watchdog detection: the mutated
    ...
    in the README so the frame is reviewable *(casebook: the liveness label that could
    swallow a finite timeout)*. When that fixture bound is offered
    ...
    of assigning `cause:liveness` *(casebook: the fixture bound that lost the race to
    PIT)*.
    ...
    *(casebook: the leaked lock with a synchronous oracle)*.
  - `cause:resource` says the mutant still completes but does too much work.
    ...
    *(casebook: the timeout whose cause still terminated)*.
  ...
  *(casebook: the liveness loop that raced the heap)*
  ...
  does not warn or fail, and no re-anchoring is required after formatting or an
  inserted method. The same-key sibling limitation is real, so positive
  multiplicity drift prints all current line-full candidates for human review
  instead of claiming a positional gate can identify the newcomer.
...
- **Duplicate rows are sibling mutants, not noise.** A compound condition
  ...
  Never hand-dedupe the file *(casebook: the sibling absorbed by its accepted
  twin)*. The verify names each extra copy `(shares an
...
2. **Identify which sibling survived.** For `RemoveConditional` pairs on
   ...
   is the one the test was aimed at *(casebook: the sibling guessed wrong three
   times)*.
   ...
   accept the row or add a redundant assertion around leaked state *(casebook: the
   leaked runner that manufactured a survivor)*.
...
- **Log emission only after contract review** — removing an incidental diagnostic
  ...
  *(casebook: the family label that made a real log contract disappear)*.
...
`VoidMethodCallMutator` *(casebook: System.out in a library factory; logEpoch: twelve
entries, one real equivalent)*.
...
- **Excluded classes never enter the population.** Every `excludedClasses`
  ...
  have not argued is still a hole, and the advisory is how you find it *(casebook: the
  advisory that named two thousand deliberate exclusions)*.
...
timeouts into load evidence *(casebook: eight per-target minutes compressed into one)*.
...
- **Trace the seam into the subject.** A `TestClock` observes only calls routed
  ...
  path receives the clock or budget the test observes *(casebook: the clock the subject
  never received)*.
...
is `@UntrackedTask` and re-runs whenever selected *(casebook: the fresh task Gradle
called up-to-date)*. Because a failed or
...
- **`RUN_ERROR` on individual mutants** — the same shape per-mutant, observed
  ...
  fresh history-free full-suite attempts with
...
the recipe no longer has to be rediscovered per repo. A failure that reproduces from a
known precondition — a `clean` first, another task rewriting state while it runs — is a
bug wearing a flake's clothing, so look for that precondition before recording a repeat
as transient *(casebook: the recompile that only failed when another compile ran)*.
...
1. Apply `software.sava.build.feature.hardening` and register mutation suites
   ...
   `.pitest-history/` git-ignore, and the adoption checklist. Run
   `./gradlew :module:hardeningHelp` for the installed
...
5. Add the agent-instructions block below to the repo's `AGENTS.md`. Run the task on
   ...
   print the block carried by the installed plugin. Then decide who owns the pre-release
...
8. For a first-party `sava-build` consumer, add its canonical GitHub slug to
   ...
   local-fuzz experiment can discover it *(casebook: the optional fleet runner that
   remained mandatory to maintain)*. Ordinary local-fuzz runs skip absent
   sibling checkouts; its opt-in strict mode refuses omissions.
## Agent instructions template

Copy the block printed by `./gradlew :module:hardeningAgentTemplate` (or
`./gradlew :hardeningAgentTemplate` when the root project owns hardening) into the
repo's `AGENTS.md`, keeping its start and end comment lines so the copy can be found
and replaced on a later upgrade. Put repository-specific facts (suite roster, who runs
`hardeningCertify`, fuzz budgets, module notes) outside that block. The block is short
by design: it lists what an agent must do and where the reasons live. Nothing checks
the copy — a stale block costs a wrong instruction, not a red build — so re-copy it
when the printed block changes, and read HARDENING.md, not the block, when a rule
needs its argument. An unqualified task name can select every hardening project in a
multi-project build and print the block once per project.
